...
from Andencento import dispatcher, WHITELIST_USERS, SUPPORT_USERS, SUDO_USERS, DEV_USERS, OWNER_ID, TOKEN
from Andencento.__main__ import IMPORTED, HELPABLE, MIGRATEABLE, STATS, USER_INFO, DATA_IMPORT, DATA_EXPORT, CHAT_SETTINGS, USER_SETTINGS 
from Andencento.modules.disable import DisableAbleCommandHandler
from Andencento.modules.log_channel import loggable, gloggable
from Andencento.modules.helper_funcs.chat_status import bot_admin, can_promote, user_admin, can_pin, sudo_plus, dev_plus, connection_status
from Andencento.modules.helper_funcs.extraction import extract_user, extract_user_and_text
...
def gitpull(bot: Bot, update: Update):
    ...
    os.system('pkill python && python3 -m Andencento')
...
def load(bot: Bot, update: Update):
    ...
    try:
        imported_module = importlib.import_module("Andencento.modules." + text)
    except:
...
def listmodules(bot: Bot, update: Update):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("Andencento.modules.", 1)[1]
